...
- `--addon` -- build mode
- `--cosign` -- keyless cosign signing via OIDC (`id-token: write` permission)
...
### Auto Release
Defined in `.github/workflows/release.yaml`. Triggers on push to `main` when `mcp-proxy/config.yaml` changes.

When the version in `config.yaml` does not have a matching GitHub release:
1. Extracts the version from `config.yaml`
2. Checks if a release for that tag already exists (idempotent)
3. Extracts the changelog section for the version from `mcp-proxy/CHANGELOG.md`
