...
3. If there's a critical rule, add a one-liner to `CLAUDE.md` with a link to the new doc
...
5. Check that the Detailed Guides section in `/CLAUDE.md` still accurately describes the doc's purpose
...
| Creating a doc without indexing it | Always add to the folder's `README.md` |
